...
const Login = () => {
  ...
  // Load saved login details on mount
  useEffect(() => {
  ...
  onSuccess: (response, variables) => {
    ...
    // ✅ API च्या message वापरा किंवा default message
    toast.success(message || "Login successful!", { position: "top-right", autoClose: 3000 });

    // Save user data in Redux
    dispatch(login({ token, user }));
    
    // Handle Remember Me in Redux (and localStorage)
    dispatch(
      RemberMe({
      ...
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg w-full max-w-md border border-gray-200">
        <div className="text-center mb-6">
          <div className="flex justify-center mb-4">
            <img src={Logo} alt="Menon Initiator Logo" className="h-16 w-auto object-contain" />
          </div>
...
{mutation.isPending ? (
  ...
  <button
    type="submit"
    className="w-full py-3 rounded-lg font-semibold text-white text-sm sm:text-base bg-orange-600 hover:bg-orange-700 active:scale-95 transition-all duration-200 shadow-md"
  >
    Login / लॉगिन
  </button>
)}
