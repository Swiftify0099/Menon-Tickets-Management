        ...
        <nav className="mt-10 md:mt-4 px-4">
          <ul className="space-y-1">
            {[
              ...
              { to: "/tickets", icon: ListChecks, label: "Tickets" },
            ].map(({ to, icon: Icon, label }) => (
              <li key={label}>
                <Link
                  to={to}
                  onClick={onClose}
                  className="flex items-center gap-3 px-4 py-2 text-sm rounded-xl hover:bg-orange-100 hover:text-orange-600 transition"
                >
                  <Icon size={20} />
                  <span>{label}</span>
                </Link>
